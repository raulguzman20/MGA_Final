...
// Salvaguarda de inicio: asegurar que 'telefono' no sea requerido en la colección 'usuarios'
async function ensureUsuariosValidator() {
  try {
    const db = mongoose.connection.db;
    const colName = 'usuarios';
    const list = await db.listCollections({ name: colName }).toArray();
    if (!list || !list[0]) {
      console.warn('[startup] Colección usuarios no encontrada para validar.');
      return;
    }
    const options = list[0].options || {};
    const currentValidator = options.validator || null;

    if (currentValidator && currentValidator.$jsonSchema) {
      const schema = JSON.parse(JSON.stringify(currentValidator.$jsonSchema));
      let needUpdate = false;
      if (Array.isArray(schema.required) && schema.required.includes('telefono')) {
        schema.required = schema.required.filter((f) => f !== 'telefono');
        needUpdate = true;
      }
      if (needUpdate) {
        const newValidator = { $jsonSchema: schema };
        await db.command({
          collMod: colName,
        ...
        console.log('[startup] Actualizado validator de usuarios: "telefono" ya no es requerido.');
      } else {
        console.log('[startup] Validator de usuarios OK ("telefono" no es requerido).');
      }
    } else {
      console.log('[startup] La colección usuarios no tiene $jsonSchema o no requiere cambios.');
    }
  } catch (e) {
    console.warn('[startup] Error al ajustar validator de usuarios:', e?.message);
  }
}
